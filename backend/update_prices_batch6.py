...
RC - Jute Natural Bag - 1509 rs. 275
...
RC - Mobile Stand with Card Holder
Black - 1701
Rs. 55
RC - Mobile Stand without Card Holder
Black - 1702
Rs. 45
...
Golden - 1706
Rs. 180
RC - Mobile Stand with Pen Stand
Black - 1707
Rs. 85
RC - Mobile Stand with Pen Stand
Silver - 1708
...
Rs. 55
RC - Rotate Plastic Mobile Stand
Black - 1710
Rs. 45
...
Rs. 60
RC - Foldable & Adjustable Mobile Stand
Black - 1714
Rs. 40
RC - Foldable & Adjustable Mobile Stand
White - 1715
...
Rs. 150
RC - Mobile Stand with Paper Holder
& Pen Stand - 1720
Rs. 150
RC - Mobile Stand without Paper Holder
with Pen Stand - 1721
Rs. 125
...
def parse_and_update():
    # Flatten the text a bit to handle cases where name and Rs. are on same or different lines
    # Let's extract all numbers that are part of product codes (e.g. 985, 1101)
    
    # Simple strategy: we know product codes in this list are numbers between 985 and 1824.
    # We can just extract all "Rs. X" and associate it with the closest preceding product code.
    ...
    for line in lines:
        ...
        # Check if line contains Rs.
        if "Rs." in line or "rs." in line.lower():
            # If it's on the same line like "RC - Keychain - 1122 Rs. 34"
    ...
    for name, price in parsed_items.items():
        ...
        # Extract the code from the name (e.g. 1101, 1401)
        # Most of them have a 3 or 4 digit code.
        code_match = re.search(r'([0-9]{3,4})', name)
        
        products = None
        if code_match:
            code = code_match.group(1)
            # Try finding by the exact code
        ...
        # If code matching fails or isn't used, try the name
        if not products or not products.exists():
            clean_name = name.replace("also set in colors", "").strip()
            products = Product.objects.filter(name__icontains=clean_name)
            
        if products and products.exists():
            for p in products:
                if price == "Call":
                    print(f"| {p.name} | Rs. Call | **Rs. Call (No Change)** |")
                else:
                    new_price = round(price * 1.60)
                    p.price = Decimal(str(new_price))
                    p.save(update_fields=['price'])
                    print(f"| {p.name} | Rs. {price} | **Rs. {new_price}** |")
        else:
            missing.append(name)
